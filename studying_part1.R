...
# import dataset
data_inrap <- readRDS(file = "output_data/final_list_ariadne_inrap.RDS")
...
write_sf(final_sf, "output_data/final_sf_descriptions_metadata.gpkg")

europ <- st_read(dsn = "data/Europe.shp")
...
ggsave(filename = "output_fig/carto.png", plot = last_plot(), device = "png", width = 20, height = 12, units = "cm")
...
# vue générale des mots les plus fréquemment utilisés dans le corpus
...
ggsave(filename = "output_fig/comparaison.png", plot = last_plot(), device = "png", width = 25, height = 18, units = "cm")
...
ggsave(filename = "output_fig/bigram.png", plot = last_plot(), device = "png", width = 18, height = 22, units = "cm")
...
# CLC data
CLC_2006 <- st_read(dsn = "data/CLCover/CLC06_FR_RGF.shp")
CLC_2018 <- st_read(dsn = "data/CLCover/CLC18_FR.shp")
referentiel_CLC_niv1 <- read_excel(path = "data/CLCover/clc-nomenclature-c_1.xls", sheet = "nomenclature_clc_niveau_1")
referentiel_CLC_niv2 <- read_excel(path = "data/CLCover/clc-nomenclature-c_1.xls", sheet = "nomenclature_clc_niveau_2")
referentiel_CLC_niv3 <- read_excel(path = "data/CLCover/clc-nomenclature-c_1.xls", sheet = "nomenclature_clc_niveau_3")
...
a <- bigram_spatial_CLC_grp %>%
  ...
  filter(n > 1) %>%
  ggplot(mapping = aes(x = reorder(libelle_reconstitue, n), y = reorder(bigrams, n), size = n, color = n)) +
  geom_point() +
  theme_bw() +
...
ggsave(filename = "output_fig/bigram-espace.png", plot = last_plot(), device = "png", width = 35, height = 22, units = "cm")

# extraction des phrases dans lesquelles on a les bigrams
bigrams_data <- final_sf %>%
  mutate(geom = geometry) %>%
  st_drop_geometry() %>%
  mutate(revue_description = str_replace_all(string = description, pattern = "\n", replacement = " ")) %>% 
  mutate(detectquartier = str_detect(string = revue_description, pattern = "quartier")) %>%
  filter(detectquartier == TRUE) %>%
  tidytext::unnest_tokens(output = sentence, input = revue_description, token = "sentences", drop = TRUE)

write.csv(x = bigrams_data, file = "output_data/bigrams_sentences.csv", row.names = FALSE)
